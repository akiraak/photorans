...
iOS の実機確認は **Debug = Mac の Xcode から直接 USB Run / Release = TestFlight 経由** の二段構成。Akira さんの開発機は Mac (macOS) で Xcode がローカルにあるため、開発中の Debug ビルドは Bitrise を経由せず Xcode から実機に直接インストールする。Release 配信前の最終確認のみ TestFlight を通す。

### Debug ローカル Run (開発中)

Mac + iPhone を同一 LAN に置き、`./server.sh` 起動済みの状態で Xcode から `ios/Photorans.xcodeproj` を開いて Run。手順とトラブルシュートは [`ios/README.md`](ios/README.md) の「Mac でローカル動作確認」節を参照。Bitrise クレジットを消費せず即時確認できる。

### Release 配信フロー (TestFlight)

Release 配信が必要な変更が main に入ったら、以下のフローで TestFlight に上げる。
...
Claude Code 側の運用:
- Debug ローカル Run で十分な変更 (UI 微調整、ログ追加、開発用フラグ等) は TestFlight に上げず Akira さんの Mac 実機確認で完結させる
- Release 配信が必要な変更 (本番ユーザに届ける機能、TestFlight ビルド番号を進める必要があるもの) は、タグ push を **Akira さんの確認を取った上で** 行う (Bitrise クレジット消費 + TestFlight に外部影響が出るため、勝手に push しない)
