...
- `AVCapturePhotoOutput` を session に追加
- シャッターボタン (SwiftUI で大きめの円形) を CameraView に追加
- 撮影 → JPEG 変換 (容量制約 5MB 以下) → `FileManager` で `Documents/photos/<uuid>.jpg` に保存
- EXIF orientation を尊重 (`CGImagePropertyOrientation` を JPEG メタに反映)
- 確認: Bitrise でビルド成功。撮影動作確認は Phase6 以降
