...
ステータス: **実装中 (Phase1〜4 完了 / Phase5 未着手)** / 開始日: 2026-04-30
...
- Apple Developer (`akiraak@gmail.com`) でログイン → App Store Connect → **Users and Access** → **Integrations** → **App Store Connect API** タブ → **Team Keys** で新規 Key 発行
  - 名前: `Bitrise Photorans` 等
  - Access: **App Manager** (`Admin` でなくて可)
  - 発行直後に `AuthKey_<KEYID>.p8` をダウンロード (一度きり)。Issuer ID / Key ID を控える
- Bitrise → Workspace settings → **Apple service connection** → API Key Auth → 上記 3 点 (Issuer ID / Key ID / `.p8`) を登録
- リポジトリの Bitrise アプリ側で **Settings → Service settings → Apple service connection** を上記コネクションに紐付け
- 確認: Bitrise UI で connection の状態が `Connected` になること

> 過去 EAS で発行した API Key があれば流用も可能。Bitrise の `Manage iOS Code Signing` Step が必要とする `App Manager` 権限を満たしているか確認すること。

#### Step2 コード署名方式の決定 (Akira さん作業 + Bitrise UI)

第一候補は **Bitrise の `Manage iOS Code Signing` Step による Automatic Code Signing**。Step1 で登録した API Key を使い、Distribution 証明書 / Provisioning Profile を自動生成・管理する。

- 旧 EAS で発行した Distribution 証明書が App Store Connect 側に残っていても支障はない (Apple は team あたり Distribution cert 2 本まで)
- 自動署名で詰まったら **手動アップロード** に切替: Apple Developer Portal で Distribution cert (`.p12`) と App Store Provisioning Profile (`com.akiraak.photorans` 用、Profile Type: `App Store`) を作成し、Bitrise の **Code Signing & Files** にアップロード

#### Step3 `release` Workflow を `bitrise.yml` に追加 (Claude 作業)

- トリガ: tag `v*` push (`trigger_map` を新設)
- 既存 `primary` Workflow は `push_branch` トリガ追加せず手動運用のまま維持
- Step 構成:
  - `git-clone@8`
  - `manage-ios-code-signing@3` — Distribution method: `app-store`、`automatic_code_signing: api-key`
  - `set-xcode-build-number@2` (任意) — `CURRENT_PROJECT_VERSION` を `$BITRISE_BUILD_NUMBER` で上書き。または env で渡す
  - `xcode-archive@5` — `project_path`、`scheme`、`configuration: Release`、`distribution_method: app-store`
  - `deploy-to-itunesconnect-application-loader@1` (= TestFlight 提出) — App Store Connect API Key 経由
  - `deploy-to-bitrise-io@2` — Bitrise の Artifacts に `.ipa`/`.dSYM` を残す
- `Info.plist` に **`ITSAppUsesNonExemptEncryption: false`** を追加 (App Store Connect の暗号化準拠申告。HTTPS のみ使用なので exempt)
- `MARKETING_VERSION` 初期値は `0.1.0` (project.yml に既に設定済み)。`CURRENT_PROJECT_VERSION` は `$BITRISE_BUILD_NUMBER` で自動採番される

#### Step4 初回 release ビルド + TestFlight 着信確認 (Akira さん作業)

- ローカルで `git tag v0.1.0 && git push origin v0.1.0`
- Bitrise の `release` Workflow が緑化することを Web UI で確認
- App Store Connect → TestFlight タブにビルドが現れ、Apple のプロセシング (5〜30 分) 後に Internal Testing グループへ自動配信
- iPhone の TestFlight アプリでビルドが見えること (実際のインストール・動作確認は Phase7)

> 詰まりポイントの想定: 自動署名失敗 (Step2 手動切替で対処) / `ITSAppUsesNonExemptEncryption` 未指定で TestFlight が "Missing Compliance" 警告 (Step3 で先回り対処) / `CFBundleVersion` が App Store Connect に既登録のものと衝突 (`$BITRISE_BUILD_NUMBER` で十分大きい値が出るので通常は発生しない)
