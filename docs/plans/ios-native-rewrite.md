...
ステータス: **実装中 (Phase1〜6 完了 / Phase7 進行中: LAN 内 1 回目確認で `/translate` 5MB 上限ヒット → 画像圧縮対応に切替 2026-05-01)** / 開始日: 2026-04-30
...
- `ios/Photorans/Features/Camera/ImageCompressor.swift` を新設、`compressForUpload(jpegData:targetBytes:)` を提供
- 元データが `targetBytes` (= 4.5MB、Anthropic 5MB の安全マージン) 以下なら無加工で返す
- 超える場合は `UIImage(data:)` 経由で段階的に再エンコード:
  1. 解像度維持で `compressionQuality` 0.85 → 0.7 → 0.5
  2. それでも超えるなら長辺 2048px に縮小して 0.85 → 0.7 → 0.5
  3. 最終フォールバック: 長辺 1600px / 0.7
- 全段階で超えた場合は最後の試行結果をそのまま返す (極端な巨大画像はほぼ来ない想定、サーバ 4xx でユーザに見せる)
- `CameraViewModel.capturePhoto` で `camera.capturePhoto` 直後に圧縮を挟み、保存 (`PhotoStorage.save`) と送信 (`TranslateAPI.translate`) の両方に圧縮後 Data を使用
- OCR 観点で 2048px 長辺 ≒ iPhone 12MP の半分程度あれば十分 (Anthropic Vision 自体も内部で 1568px 前後にリサイズする)
