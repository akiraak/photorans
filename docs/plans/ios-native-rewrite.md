...
ステータス: **実装中 (Phase1〜6 完了 / Phase7 進行中: `v0.1.2` 配布で base64 サイズ前提が誤りと判明、Step5b で再調整中 2026-05-01)** / 開始日: 2026-04-30
...
#### Step2 タグ `v0.1.1` push → release Workflow → TestFlight 配布 (✅ 完了 2026-05-01)

- `git tag v0.1.1 df6415a && git push origin v0.1.1` で `release` Workflow をトリガ
- Bitrise の `release` Workflow が緑化、`.ipa` が `deploy-to-itunesconnect-application-loader@1` で TestFlight Internal Testing に到達
- iPhone の TestFlight アプリで `v0.1.1` をインストール完了

> Internal Testing は Apple Review 不要なので、Release 構成でも HTTP の LAN URL を埋め込んだビルドを配布可能。

#### Step3 LAN 内 1 回目動作確認 (✅ 完了 2026-05-01、詰まり発覚)

- iPhone を WSL2 ホストと同じ Wi-Fi に接続して撮影 → サーバ `/translate` で **Anthropic Vision API が 5MB 上限超過の 400 エラー** を返した
- サーバログ:
  ```
  anthropic api error: status=400 message=400 {"type":"error","error":{"type":"invalid_request_error","message":"messages.0.content.0.image.source.base64: image exceeds 5 MB maximum: 6583144 bytes > 5242880 bytes"},"request_id":"req_011CacShANVCtzpmW5isvcSi"}
  ```
- iPhone カメラの `.photo` preset + `AVCapturePhotoSettings` のデフォルト品質では JPEG 6.5MB 程度になり、Anthropic Vision の 5MB 制限を恒常的に超える
- Phase2 Step2 のメモ「過大画像が問題になったら別タスクで圧縮ロジックを追加」の前提が顕在化したため、Step4 で対応する

#### Step4 撮影画像のクライアント側自動圧縮 (✅ 完了 2026-05-01)

撮影直後の `Data` を「Anthropic Vision 5MB 上限を満たし、かつ OCR で読める解像度を保つ」JPEG に再エンコードしてから保存・送信する。サーバ側ではなくクライアント側で対応する理由:

- Anthropic Vision の 5MB 制約はクライアント実装で完結して守るのが筋 (サーバは透過プロキシでよい)
- ネットワーク帯域削減 (Phase8 でモバイル回線時にも効く)
- 履歴に保存する画像も同じく圧縮済みでよい (OCR 用の元画像なので品質はそこそこで十分)

実装内容:

- **`ios/Photorans/Features/Camera/ImageCompressor.swift` 新設** (✅ 完了): `enum ImageCompressor` の static func `compressForUpload(jpegData:targetBytes:)` を提供。`defaultTargetBytes = 4_500_000` (Anthropic 5MB の安全マージン)
- **段階的再エンコード** (✅ 完了): 元データが `targetBytes` 以下なら無加工で返す。超える場合は `UIImage(data:)` 経由で:
  1. 解像度維持で `compressionQuality` 0.85 → 0.7 → 0.5
  2. 超過時は長辺 2048px (`UIGraphicsImageRendererFormat`、scale=1, opaque=true) に縮小して 0.85 → 0.7 → 0.5
  3. 最終フォールバック: 長辺 1600px / 0.7
- **全段階超過時** (✅ 完了): 最後の試行 (= 解像度維持の quality 0.5、3 のリサイズが nil の場合) を返す。極端な巨大画像はほぼ来ない想定でサーバ 4xx に委ねる
- **`CameraViewModel.capturePhoto` 統合** (✅ 完了): `camera.capturePhoto` 直後に圧縮を挟み、保存 (`PhotoStorage.save`)・サムネ (`UIImage(data:)`)・送信 (`translate(jpegData:)`) すべてに圧縮後 Data を使用
- **xcodegen 再生成** (✅ 完了): `cd ios && xcodegen generate` で `ImageCompressor.swift` が `Photorans` ターゲットの Camera グループ + Sources ビルドフェーズに追加されたことを `project.pbxproj` 上で確認
- 確認: WSL2 上のビルド検証は不可。Bitrise の `release` Workflow ビルドは Step5 のタグ push で確認 (実機での圧縮効果検証は Step6 で実施)

#### Step5 タグ `v0.1.2` push → release Workflow → TestFlight 再配布 (✅ 完了 2026-05-01)

- `v0.1.2` を `9664b6a` に打って push、Bitrise の `release` Workflow 緑化
- TestFlight Internal Testing に build 13 (= App Store Connect 上の `0.1.0(13)`) が到達、iPhone でインストール完了
- ただし実機で再撮影した結果、`/translate` で再度 Anthropic Vision の 400 エラー (`image exceeds 5 MB maximum: 5444424 bytes`) → Step5b で対応

#### Step5b 圧縮しきい値再調整 + `v0.1.3` 再配布

Step4 で `defaultTargetBytes = 4_500_000` (raw 4.5 MiB) としたが、**Anthropic Vision の 5 MiB 制限は base64 文字列長に対するもの** と v0.1.2 実機検証で判明した。base64 は raw の 4/3 倍に膨らむため、raw を 3.75 MiB 未満に抑える必要がある。

`ImageCompressor` の戦略を以下に書き換える:

- `defaultTargetBytes = 1_500_000` (raw 1.43 MiB → base64 ~2 MiB、5 MiB 上限に十分マージン)
- `defaultMaxLongestEdge = 2048` を新設し、**常に長辺 ≤ 2048px に縮小** (Anthropic Vision は内部で ~1568px に再リサイズするため OCR 精度損失は微小)
- 縮小後に quality 0.8 → 0.65 → 0.5 を順試行
- フォールバック: 長辺 1280px / 0.5
- 全段階で超えた場合は最終フォールバック相当 (縮小後 quality 0.35) を返す

ファイル変更:

- `ios/Photorans/Features/Camera/ImageCompressor.swift` を上記戦略に置き換え (シグネチャ `compressForUpload(jpegData:targetBytes:maxLongestEdge:)` に拡張)

確認:

- WSL2 上のコンパイル検証は不可。`v0.1.3` タグを HEAD に打って Bitrise → TestFlight 配布
- iPhone TestFlight で更新後、実機で `/translate` 200 OK + サーバ `/admin` の画像欄 (`record.imagePath`) で実画像が ~300-800KB 程度に収まっていること
- Step6 へ移行

#### Step6 LAN 内動作再確認

- 撮影 → `/translate` 200 OK → 履歴一覧に反映 → 詳細表示 → コピー / 削除
- WSL2 サーバの `/admin` に履歴が積まれること
...
- タグ `v0.1.3` push で `release` Workflow → TestFlight 配布 (`v0.1.2` は Phase7 Step5 の画像圧縮版で消化済み)
- iPhone の TestFlight で `v0.1.3` に更新
...
- モバイル回線の細い帯域でも撮影 → 翻訳が許容できる時間内に通ること (Phase7 Step4 圧縮の効き目確認も兼ねる)
