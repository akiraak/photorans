...
#### Step5b 圧縮しきい値再調整 + `v0.1.3` 再配布 (✅ 完了 2026-05-01)
...
- `ios/Photorans/Features/Camera/ImageCompressor.swift` を上記戦略に置き換え (シグネチャ `compressForUpload(jpegData:targetBytes:maxLongestEdge:)` に拡張) → コミット `ab659b9`
- 加えてサーバ側 `server/src/index.ts` に `[translate]` ログを追加 (圧縮後サイズ + Anthropic レスポンスの可視化、コミット `b0f84b8`)

確認 (✅ 完了 2026-05-01):

- `v0.1.3` タグ push → Bitrise `release` Workflow 緑 → TestFlight 配布
- iPhone TestFlight で実機確認、Step6 で OK 判定済み

#### Step6 LAN 内動作再確認 (✅ 完了 2026-05-01)
