...
- **Phase2 プレビュー回転対応** — `CameraPreviewView` の `connection.videoRotationAngle` を端末向きに追従させる
...
- **Phase3 撮影 UI の回転追従 + landscape レイアウト** — UI 自体は portrait 固定だが、シャッター位置とアイコン向きを landscape の余白に合わせる
  - Step3-1 `CameraView` の GeometryReader 内で preview の余白領域 (portrait なら下、landscape なら右) を計算して bottomControls の配置を切替
  - Step3-2 `shutterButton` / `thumbnailView` / `translatingOverlay` に `.rotationEffect(.degrees(rotation))` + `.animation` を適用 (rotation は `lastValidRotationAngle` を 90 / 0 / 180 のいずれかから供給。upside-down / faceUp / faceDown は B3 補足の通り無視)
...
判断 A / B / E は Phase1 着手時にユーザーに最終確認する (本ドキュメント上は (A2) + (B3) + (E1) を第一候補として進める)。
