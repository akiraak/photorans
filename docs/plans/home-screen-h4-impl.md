...
- 親プラン: `docs/plans/screen-architecture.md` (画面構成パターン A 採用、Root 構成 H4 採用)
- 仕様プラン: `docs/plans/home-screen-h4-spec.md` (S1〜S14 の挙動を確定済み)
...
- **Step 2.4** — `Features/Home/UnclassifiedListView.swift` を追加。`init(scope:)` で `Predicate<Item>` を組み立てて `_query = Query(filter: ..., sort: \Item.createdAt, order: .reverse)` に注入 (Root: `group == nil` / Group X: `group?.id == X.id`)。Step 0.1 の PoC で optional keypath が通らなかった場合は **`ItemGroup.items` 直読みに倒し、in-memory ソート** に切り替える (S3-2)。空状態は `ContentUnavailableView` 文言 S11 を採用 (✅)
- **Step 2.5** — `Features/Home/GroupListView.swift` を追加。`init(scope:)` で `Predicate<ItemGroup>` で `parent` をフィルタした子 Group を取得 (Step 0.1 の方針に従う)。並び順は **直下 Item の最新 createdAt で降順、直下 Item ゼロの中間 Group は `nil` 扱い → 末尾固定** とし、ソートは `@Query` ではなく取得後の in-memory で行う (関係越しの max は SwiftData Predicate で表現困難なため)。空状態は ContentUnavailableView (S11) (✅)
...
- **Step 0.1** `Predicate<ItemGroup>` の optional keypath: ☐ 未検証 (Phase 0 で埋める)
- **Step 0.2** Background `ModelContext` × strict concurrency: ☐ 未検証
- **Step 0.3** NavigationStack destination 集約: ☐ 未検証
- **Step 0.4** deleteRule 確定: ☑ 仕様プラン S9 を `.cascade` に修正済み (2026-05-02、本プラン更新と同時に反映完了)

## 次のステップ

1. 本プランをユーザーレビュー (Phase / Step の粒度、リスク項目への合意)
2. レビュー指摘を反映したら、`TODO.md` Phase 4 をチェックして Phase 5 を起こす
3. impl Phase 0 (PoC) から順に着手し、PoC 結果を本プラン末尾に追記してから Phase 1 に入る
