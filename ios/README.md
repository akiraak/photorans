...
WSL2 Ubuntu 上では Apple SDK が無くても XcodeGen 自体はビルドできる。Swift toolchain (Linux) を使ってソースビルドする。現在は Mac 単独運用に移行済みだが、将来再度 Linux で再生成する場合の手順として残す。

```bash
# Swift toolchain → swiftly init → . env.sh
...
ローカル Debug ビルド / 実機 USB Run は Mac の Xcode で行う (下記「Mac でローカル動作確認」参照)。CI / Release 配信は Bitrise (`bitrise.yml` の `primary` = simulator ビルド検証 / `release` = TestFlight 配信) を介する。

| Configuration | `API_BASE_URL`                       |
| ------------- | ------------------------------------ |
| Debug         | http://10.0.1.221:3000/$(URL_SECRET) |
| Release       | https://photorans.chobi.me/$(URL_SECRET) |
